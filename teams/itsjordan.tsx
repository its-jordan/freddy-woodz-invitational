...
export default async function Itsjordan() {
  const api = new PokemonClient();
  let playerName = 'its_jordan';
  let pok1 = await api.getPokemonByName('scizor-mega');
  let pok2 = await api.getPokemonByName('blaziken');
  let pok3 = await api.getPokemonByName('infernape');
  let pok4 = await api.getPokemonByName('torkoal');
  let pok5 = await api.getPokemonByName('quagsire');
  let pok6 = await api.getPokemonByName('sableye');
  let pok7 = await api.getPokemonByName('flygon');
  let pok8 = await api.getPokemonByName('mismagius');
  let pok9 = await api.getPokemonByName('kecleon');
  let pok0 = await api.getPokemonByName('probopass');
