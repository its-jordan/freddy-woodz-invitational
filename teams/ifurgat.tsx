...
export default async function Ifurgat() {
  const api = new PokemonClient();
  let playerName = 'ifurgat';
  let pok1 = await api.getPokemonByName('lopunny-mega');
  let pok2 = await api.getPokemonByName('landorus-therian');
  let pok3 = await api.getPokemonByName('starmie');
  let pok4 = await api.getPokemonByName('gengar');
  let pok5 = await api.getPokemonByName('cobalion');
  let pok6 = await api.getPokemonByName('moltres');
  let pok7 = await api.getPokemonByName('persian-alola');
  let pok8 = await api.getPokemonByName('accelgor');
  let pok9 = await api.getPokemonByName('heliolisk');
  let pok0 = await api.getPokemonByName('carbink');
