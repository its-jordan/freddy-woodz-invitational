...
export default async function Foxish() {
  const api = new PokemonClient();
  let playerName = 'foxish';
  let pok1 = await api.getPokemonByName('gliscor');
  let pok2 = await api.getPokemonByName('alakazam-mega');
  let pok3 = await api.getPokemonByName('tornadus-incarnate');
  let pok4 = await api.getPokemonByName('zoroark-hisui');
  let pok5 = await api.getPokemonByName('chesnaught');
  let pok6 = await api.getPokemonByName('kingdra');
  let pok7 = await api.getPokemonByName('porygon2');
  let pok8 = await api.getPokemonByName('slowking');
  let pok9 = await api.getPokemonByName('avalugg-hisui');
  let pok0 = await api.getPokemonByName('luxray');
