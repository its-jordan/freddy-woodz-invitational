...
export default async function Resolamxxy() {
  const api = new PokemonClient();
  let playerName = 'resolamxxy';
  let pok1 = await api.getPokemonByName('manaphy');
  let pok2 = await api.getPokemonByName('garchomp');
  let pok3 = await api.getPokemonByName('azelf');
  let pok4 = await api.getPokemonByName('breloom');
  let pok5 = await api.getPokemonByName('salamence');
  let pok6 = await api.getPokemonByName('politoed');
  let pok7 = await api.getPokemonByName('gastrodon');
  let pok8 = await api.getPokemonByName('ampharos-mega');
  let pok9 = await api.getPokemonByName('kabutops');
  let pok0 = await api.getPokemonByName('claydol');
