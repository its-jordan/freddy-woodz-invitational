...
export default async function Tokotoro() {
  const api = new PokemonClient();
  let playerName = 'Tokotoro';
  let pok1 = await api.getPokemonByName('swampert-mega');
  let pok2 = await api.getPokemonByName('ferrothorn');
  let pok3 = await api.getPokemonByName('pelipper');
  let pok4 = await api.getPokemonByName('thundurus-incarnate');
  let pok5 = await api.getPokemonByName('goodra-hisui');
  let pok6 = await api.getPokemonByName('diancie');
  let pok7 = await api.getPokemonByName('umbreon');
  let pok8 = await api.getPokemonByName('primeape');
  let pok9 = await api.getPokemonByName('qwilfish');
  let pok0 = await api.getPokemonByName('drifblim');
